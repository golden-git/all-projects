...
timber.productPage = function (options) {
  ...
  var $productImage = $('#product-featured-image'),
     ...
     // $quantityElements = $('.quantity-selector, label + .js-qty, .qtyminus, .qtyplus, .singlepro__quantity'),
  ...
  if (variant) {
    ...
    $productPrice.html( safa.formatMoney(variant.price, moneyFormat) );

    // Also update and show the product's compare price if necessary
    if (variant.compare_at_price > variant.price) {
      $comparePrice
        .html(safa.formatMoney(variant.compare_at_price, moneyFormat))
        .show();
    } else {
      $comparePrice.hide();
    }
    ...
    //   - http://docs.safa.com/manual/configuration/store-customization/advanced-navigation/linked-product-options
    $addToCart.addClass('disabled').prop('disabled', true);
  ...
  /*begin variant image*/
    if (variant && variant.featured_image) {
        ...
        safa.Image.switchImage(newImage, element, function (newImageSizedSrc, newImage, element) {
